...
La fonction \proc{update\_cost\_table}(image, costTable, groove) permet de mettre à jour la table des coûts \id{costTable} sur base du sillon \id{groove} qui a été retiré de l'image.
...
\begin{codebox} %%Explication du package : https://www.cs.dartmouth.edu/~thc/clrscode
	    ...
	    \li \For $number \gets 1$ \To \id{k} \Comment \id{k} $\times$
                \Do
         \li    	\id{optimalGroove} $\gets$ \func{find\_optimal\_groove}(nCostTable) \Comment $\Theta(\id{n} \times \id{m})$
...
Si on additionne les complexités temporelles, on obtient $\mathcal{O}((2+3\id{k})(\id{n} \times \id{m}) + 2)$pour la complexité en temps.\\
\\
\noindent Concernant les complexités spatiales :

\begin{enumerate}
	\item[$\bullet$] Stocker la copie de l'image représente une complexité spatiale de $\Theta(\id{n} \times \id{m})$.
